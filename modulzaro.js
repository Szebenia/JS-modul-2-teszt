...
const printCheaperThan = (userPrice, products) =>
  console.log(products.filter((product) => product.price < userPrice));
...
//   const cheapestPrice = products.reduce((accumulator, product) =>
//     product.price > accumulator ? accumulator : product.price
//   );
...
const result = getCheapestProduct(shopData);
...
function getUniqueCategories(stuff) {
  function getAllCategories(products) {
    let allCategories = [];
    allCategories = products.map((product) => product.category);

    return allCategories;
  }

  const allCat = getAllCategories(stuff);

  const clearCatList = allCat.reduce((accumulator, item) => {
...
function drawCategories(categoriesList) {
  const productsDiv = document.getElementById("products");
  categoriesList.forEach((category) => {
    productsDiv.insertAdjacentHTML(
      "beforeBegin",
      `<div class="categories-container">
        <button class="category-btn" onclick="goRed(event)">${category}</button>
    </div>`
    );
  });
}
...
function goRed(event) {
  ...
  const otherBtns = document.getElementsByClassName("category-btn");
  for (btn of otherBtns) {
    if (btn != event.target) {
...
function drawCategories2(categoriesList) {
  const productsDiv = document.getElementById("products");
  categoriesList.forEach((category) => {
    productsDiv.insertAdjacentHTML(
      "beforeBegin",
      `<div class="categories-container">
          <button class="category-btn" onclick="goRed2(event)">${category}</button>
      </div>`
    );
  });
}

function goRed2(event) {
  const currentBtn = event.target;
  currentBtn.classList.toggle("active");
  const currentBtnCat = currentBtn.innerText;

  let selectedItem = shopData.filter(
    (product) => product.category === currentBtnCat
  );
  const productsDiv = document.getElementById("products");
